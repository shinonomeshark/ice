...
public class UnicodeFontRenderer {
    ...
    public final int FONT_HEIGHT = 6;
    ...
    private Font getFontByName(String name) throws IOException, FontFormatException {
        return getFontFromInput("/assets/hackage/fonts/" + name + ".otf");
    }
...
//        y = y+2f;
    ...
    public int drawChromaString(String text, float x, float y, int color) {
        if (text == null)
            return 0;
...
//        GlStateManager.color(red, green, blue, alpha);
        ...
        int i = -1;
        float xTmp = x;
        for (char textChar : characters) {
////            float l = ((-System.nanoTime() +1000 % 1000)/1000f);
//            i = Color.HSBtoRGB((l+(l/2))/5, 1f, 1);
            float hue = ((-System.currentTimeMillis() +1000 % 1000)/1000f);
//            int color111 = Color.HSBtoRGB(34,1,1);
//            float l = (System.currentTimeMillis() - (xTmp * (System.currentTimeMillis()/xTmp) - y * 101));
//            int color111 = Color.HSBtoRGB((float)(System.currentTimeMillis() % 8000L) / 4000.0F, 0.6F, 0.8F);
//            int ii = Color.HSBtoRGB(l % (int) 2000.0F / 2000.0F, 0.8F, 0.8F);
            long l = (long) (System.currentTimeMillis() - (xTmp * 10 - y * 10));
            int ii = Color.HSBtoRGB(l % (int) 2000.0F / 2000.0F, 0.5F, 0.8F);
            //            System.out.println();
            System.out.println(ii);
            
            
            org.newdawn.slick.Color col = new org.newdawn.slick.Color(ii);
            String tmp = String.valueOf(textChar);
            unicodeFont.drawString(xTmp, y, tmp, col);
            xTmp += unicodeFont.getWidth(""+textChar);
//            index++;
        }
