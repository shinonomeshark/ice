...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
   ...
   public void handleEntityVelocity(S12PacketEntityVelocity packetIn) {
      ...
      if(entity != null) {
    	 if(!Client.modOn(new Velocity().name)) {
//    		 Client.addChat("done");
   ...
   public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
	   if(!Client.moddedFlying) {
	      ...
	      if(packetIn.func_179834_f().contains(S08PacketPlayerPosLook.EnumFlags.X_ROT)) {
	         ...
	         if(Client.modOn(new NoRotateSet().name)) {
	        	 f1= Minecraft.getMinecraft().thePlayer.rotationPitch;
	         }
	         Client.lagDetect();
	      }
   ...
   public void handleChat(S02PacketChat packetIn) {
      ...
      if(packetIn.getType() == 2) {
         ...
         if(packetIn.getChatComponent().getUnformattedText().contains("by " + Minecraft.getMinecraft().thePlayer.getName())) {
//        	 Client.addChat("sus");
...
//    	    System.out.println(generatedString);
        	 ...
        	 Minecraft.getMinecraft().thePlayer.sendChatMessage("get tapped by pine v4___________" + UUID.randomUUID().toString());
         }
      }
   }
